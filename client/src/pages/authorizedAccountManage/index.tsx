...
export default function AuthorizedAccountManage() {
    const accountColumn: PrimaryTableCol[] = [
        ...
        },
        {
            ...
            title: "授权时间",
            render: ({ row }) =>
                moment(row.authTime).format("YYYY-MM-DD HH:mm:ss"),
        },
        {
            align: "center",
            minWidth: 100,
        ...
        },
        {
            ...
            render: ({ row }) =>
                row.basicConfig
                    ? row.basicConfig.isPhoneConfigured
                        ? "已绑定"
                        : "未绑定"
                    : "-",
        },
        {
            ...
            render: ({ row }) =>
                row.basicConfig
                    ? row.basicConfig.isEmailConfigured
                        ? "已绑定"
                        : "未绑定"
                    : "-",
        },
        {
            ...
            title: "认证类型",
            cell: ({ row }) => {
                return row.appType === 0
                    ? miniProgramAuthType[String(row.verifyInfo)]
                    : officialAccountAuthType[String(row.verifyInfo)];
            },
        },
        {
            align: "center",
            ...
            render({ row }) {
                if (row.accountStatus === 16) {
                    ...
                    return (
                        <div style={{ width: "210px" }}>
                            <PopConfirm
                                content="从数据库获取 token，非重新生成token，不会导致上一个 token 被刷新而失效"
                                onConfirm={() => createToken(row.appid)}
                            >
                                <a className="a" style={{ margin: "0 10px" }}>
                                    获取token
                                </a>
                            </PopConfirm>
                            <a
                                className="a"
                                onClick={() => copyMessage(row.refreshToken)}
                            >
                                复制refresh_token
                            </a>
                        </div>
                    ...
                    >
                        {row.releaseInfo && row.funcInfo.includes(17) && (
                            <a
                                ...
                                onClick={() => getMiniProgramCode(row.appid)}
                            >
                                获取小程序码
                            </a>
                        )}
                        {row.serviceStatus === 0 && (
                            <a
                                className="a"
                                style={{ marginRight: "5px" }}
                                onClick={() => openServiceStatus(row.appid)}
                            >
                                恢复服务
                            </a>
                        )}
                        <a
                            className="a"
                            href={`#${routes.miniProgramVersion.path}?appId=${row.appid}`}
                        >
                            版本管理
                        </a>
                    </div>
    ...
    const [miniProgramAppIdInput, setMiniProgramAppIdInput] = useState<
        string | number
    >("");
    ...
    const refreshAuthorizedAccount = async () => {
        const resp = await request({
            request: refreshAuthorizedListRequest,
        });
        if (resp.code === 0) {
        ...
        <div>
            ...
            >
                ...
                <p style={{ marginLeft: "8px" }}>
                    <Button
                        ...
                        variant="outline"
                        size="small"
                        onClick={refreshAuthorizedAccount}
                    >
                        <RefreshIcon />
                    </Button>
                </p>
            ...
            <div className="normal_flex">
                ...
                <p className="desc" style={{ margin: 0 }}>
                    授权帐号指的是获得公众号或者小程序管理员授权的帐号，服务商可为授权帐号提供代开发、代运营等服务。
                </p>
            </div>
            <div className="normal_flex">
                <div className="blue_circle" />
            ...
            >
                <TabPanel value={tabs[0].value} label={tabs[0].label}>
                    <Input
                        ...
                        placeholder="请输入 AppID，不支持模糊搜索"
                        suffixIcon={
                            <a className="a" onClick={getAccountList}>
                                <SearchIcon />
                            </a>
                        }
                    />
                ...
                <TabPanel value={tabs[1].value} label={tabs[1].label}>
                    <Input
                        value={miniProgramAppIdInput}
                        onChange={setMiniProgramAppIdInput}
                        style={{ width: "400px", margin: "10px 0" }}
                        placeholder="请输入 AppID，不支持模糊搜索"
                        suffixIcon={
                            <a className="a" onClick={getMiniProgramList}>
                                <SearchIcon />
                            </a>
                        }
                    />
            ...
            <Dialog
                header="AuthorizerAccessToken"
                visible={visibleTokenModal}
                footer={null}
                onClose={() => setVisibleTokenModal(false)}
            >
                <Table
                    data={tokenData}
                    columns={tokenColumn}
