...
export default function MiniProgramVersion() {
  ...
  useEffect(() => {
    if (!localExtJson || !appId || !formRef.current || !visibleSubmitModal) {
      return;
    }
  ...
  const auditOptions = useMemo(() => {
    ...
    if (
      ...
      versionData.auditInfo?.status === 3
    ) {
      return [
        {
          content: "提交审核",
    ...
    if (
      versionData.auditInfo?.status === 2 ||
      versionData.auditInfo?.status === 4
    ) {
      return [
        {
          content: "加急审核",
  ...
  const submitCode = async (e: { validateResult: any }) => {
    ...
    const { templateId, extJson, userVersion, userDesc } =
      formRef.current.getAllFieldsValue();
    const resp = await request({
      request: {
            ...
            <div style={{ display: "flex" }}>
              ...
              >
                ...
                <p style={{ fontSize: "16px" }}>
                  {versionData.releaseInfo.releaseVersion}
                </p>
              </div>
              <div
                style={{
              ...
              >
                <div className="normal_flex">
                  <p className="desc" style={{ width: "74px" }}>
                    发布时间
                  </p>
                  <p>
                    {moment(versionData.releaseInfo.releaseTime).format(
                      "YYYY-MM-DD HH:mm:ss"
                    )}
                  </p>
                </div>
      ...
      <div className="normal_flex">
        ...
        {versionData.auditInfo && (
          <div style={{ margin: "30px 10px 18px 10px" }}>
            {versionData.auditInfo.status === 0 && (
              <Tag theme="success">审核通过</Tag>
            )}
            {versionData.auditInfo.status === 1 && (
              <Tag theme="danger">审核不通过</Tag>
            )}
            {versionData.auditInfo.status === 2 && (
              <Tag theme="warning">审核中</Tag>
            )}
            {versionData.auditInfo.status === 3 && (
              <Tag theme="primary">已撤回</Tag>
            )}
            {versionData.auditInfo.status === 4 && (
              <Tag theme="warning">审核延后</Tag>
            )}
          </div>
        )}
      </div>
          ...
          <div>
            <div style={{ display: "flex" }}>
              ...
              >
                <p className="desc">版本号</p>
                <p style={{ fontSize: "16px" }}>
                  {versionData.auditInfo.userVersion}
                </p>
              </div>
              <div
                style={{
              ...
              >
                ...
                <div className="normal_flex">
                  ...
                  <p>
                    {moment(versionData.auditInfo.submitAuditTime).format(
                      "YYYY-MM-DD HH:mm:ss"
                    )}
                  </p>
                </div>
                ...
                {versionData.auditInfo.status === 1 && (
                  <Alert
                    ...
                    icon={<span />}
                    maxLine={0}
                    message={`【驳回原因】：${versionData.auditInfo.reason}`}
      ...
      {versionData.expInfo ? (
          ...
          <div>
            <div style={{ display: "flex" }}>
              ...
              >
                <p className="desc">版本号</p>
                <p style={{ fontSize: "16px" }}>
                  {versionData.expInfo.expVersion}
                </p>
              </div>
              <div
                style={{
              ...
              >
                <div className="normal_flex">
                  ...
                  <p>
                    {moment(versionData.expInfo.expTime).format(
                      "YYYY-MM-DD HH:mm:ss"
                    )}
                  </p>
                </div>
        ...
        <Loading size="small" loading={loading} showOverlay>
          <div style={{ textAlign: "center", margin: "100px 0" }}>
            ...
            <Button onClick={() => setVisibleSubmitModal(true)}>
              提交代码
            </Button>
          </div>
        </Loading>
      ...
      >
        <Form ref={formRef} onSubmit={submitCode} labelWidth={200}>
          <FormItem
            ...
            help="第三方平台小程序模板库的模板id。需从开发者工具上传代码到第三方平台草稿箱，然后从草稿箱添加到模板库。当前仅支持普通模板，尚未支持标准模板"
            rules={[
              {
                required: true,
                message: "模板id必选",
          ...
          >
            <Select
              style={{ width: "300px" }}
              popupProps={{ overlayStyle: { width: "350px" } }}
              onChange={(e) => {
                ...
                if (currentTemplate) {
                  formRef.current.setFields([
                    {
                      name: "userDesc",
                      // 删除userDesc开头的[]以及[]中的内容
                      value: currentTemplate.userDesc.replace(/\[.*?\]/g, ""),
                    },
                    {
                      name: "userVersion",
                      value: currentTemplate.userVersion,
                    },
                  ]);
                }
              }}
            >
              ...
              {[
                ...
                templateList.map((i) => {
                  return (
                    <Option
                      key={`template_${i.templateId}`}
                      value={i.templateId}
                      label={`ID：${i.templateId}`}
                      onSelect={(e) => {
          ...
          <div className="t-form__controls" style={{ marginLeft: "200px" }}>
            <div className="t-form__help">
              用于控制ext.json配置文件的内容的参数{" "}
              <a
                className="a"
